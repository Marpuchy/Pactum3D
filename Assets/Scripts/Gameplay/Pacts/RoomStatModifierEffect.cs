...
[CreateAssetMenu(fileName = "RoomStatModifierEffect", menuName = "Gameplay/Pacts/Room Modifier Effect")]
public sealed class RoomStatModifierEffect : RoomPactEffect
{
    ...
    [SerializeField] private RoomParamType statType;
    ...
    public override void Apply(RoomParamQuery query)
    {
        if (query == null)
            return;
    ...
    private bool HasRequiredTags(RoomParamQuery query)
    {
    ...
    protected override string BuildAutoDescription()
    {
        ...
        string baseText = operation == Operation.AddFlat
            ? $"Room {valueText} {statLabel}"
            : $"Room {statLabel} {valueText}";
