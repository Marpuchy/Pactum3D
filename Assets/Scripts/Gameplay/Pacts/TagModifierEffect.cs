...
[CreateAssetMenu(fileName = "TagModifierEffect", menuName = "Gameplay/Pacts/Tag Modifier Effect")]
public sealed class TagModifierEffect : EnemyPactEffect
{
    ...
    private bool MatchesTargetFilter(EnemyStatQuery query)
    {
    ...
    private bool MatchesIncludeTags(EnemyStatQuery query)
    {
    ...
    private bool MatchesExcludeTags(EnemyStatQuery query)
    {
    ...
    protected override string BuildAutoDescription()
    {
        ...
        if (filters.Length == 0)
            return $"Enemy tags: {changes}";

        return $"Enemy tags: {changes} ({filters})";
    }
