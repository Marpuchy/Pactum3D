...
public static class PlayerMovementAgentTypeMapper
{
    ...
    public static void Apply(GameObject playerInstance, IReadOnlyList<GameplayTag> tags)
    {
        if (playerInstance == null)
            return;

        if (!playerInstance.TryGetComponent(out NavMeshAgent agent))
            return;
        ...
        if (!TryGetAgentTypeId(targetAgentType, out int targetAgentTypeId))
        {
            ...
            {
                Debug.LogWarning(
                    $"PlayerMovementAgentTypeMapper: NavMesh Agent Type '{targetAgentType}' no existe en Navigation settings.",
                    playerInstance);
            }
        ...
        {
            EnsureBoundToAgentNavMesh(agent, playerInstance.transform.position, targetAgentTypeId);
            return;
        }
